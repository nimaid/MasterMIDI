...
parser.add_argument('-g', '--generate', help =
    'Length of text to generate at the end of each epoch. Defaults to 600.',
    required = False, default = 600, nargs = 1, type = int)
parser.add_argument('-e', '--epochs', help =
    'Number of epochs to train. Default is 250.',
    required = False, default = 250, nargs = 1, type = int)
...
parser.add_argument('-l', '--layers', help =
...
if args['epochs'] is not 250: 
    epochs = max(1, args['epochs'][0]) # default 250 is set in .add_argument above if not set by user
...
if args['reportrate'] is not 25: 
    report_rate = max(1, args['reportrate'][0]) # default 25 is set in .add_argument above if not set by user
